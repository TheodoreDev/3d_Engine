...
class vec2:
    ...
    def toScreen(self):
        return vec2(((29/13.5) * eg.height/eg.width * self.x+1)*eg.width/2,
                    (-self.y+1)*eg.height/2)
...
class vec3:
    ...
    def projection(self, flength):
        return flength*vec2(self.x, self.y)/self.z
...
class Triangle3D:
    ...
    def projection(self, flength):
        return Triangle2D(self.v1.projection(flength), self.v2.projection(flength), self.v3.projection(flength))
...
def linePlaneCol(plane_norm, plane_point, v1, v2):
    u = v2 - v1
    dotp = dot(plane_norm, u)
    if abs(dotp) < 1e-5:
        return (0, 0, 0)
    w = (v1 - plane_point)
    si = -dot(plane_norm, w)/dotp
    u = si*u
    return (v1 + u)
